            ...
            if (!(preg_match('~[0-9]~', $_POST['last_name']))) {
                if (filter_var($_POST['userEmail'], FILTER_VALIDATE_EMAIL)) {
                    ...
                    if (!empty($_POST['user_pass'])) {
                        ...
                        } else $passerror = "<p style='border-width:1px !important' class='text-danger border border-danger text-center mt-4'><i class='fas fa-times-circle'></i>&nbsp;&nbsp;ERROR: PASSWORD MUST BE 6 CHARS LONG, MUST CONTAIN AT LEAST ONE UPPERCASE, ONE LOWERCASE LETTER AND NUMBER</p>";
                    ...
                    if (!empty($_FILES['profile_img']['name'])) {
                        ...
                        try {
                            ...
                            $image_name = "xyzabcdef.jpg";
                        }

                        $image_name .= ".jpg";
                        $temp_path = $profile['tmp_name'];

                        //Image extension should be in given format
                        if (preg_match('*.jpg|.png|.jpeg*i', $profile['name'])) {
                            $actual_path = 'C:\xampp\htdocs\website\project\panels\trader-panel\profile\profile-img\\' . $image_name . '';
                    ...
                    $query .= " WHERE USERS.USER_ID = ".$_SESSION['trader']."";
                    ...
                    if ($ups) {
                        ...
                        <script>
                            setTimeout(() => {
                                window.location.href = '/website/project/panels/trader-panel/profile/trader-profile.php';
                            }, 2000)
                        </script>
